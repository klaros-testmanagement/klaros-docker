...
|ENV JAVA_OPTS -Duser.timezone=Europe/Berlin  |-Duser.timezone=Europe/Berlin  | With JAVA_OPTS additional settings can be transferred to the TomCat server. In this example the time zone is set to Europe/Berlin.
|ENV KLAROS_VER ${KLAROS_VER:-4.12.1}         |${KLAROS_VER:-4.12.1} |Klaros version used during installation. Versions can be found on the Official https://www.klaros-testmanagement.com/en_US/download[Klaros Test Management] website or on https://github.com/klaros-testmanagement/klaros-docker/releases[GitHub]
|=======================
