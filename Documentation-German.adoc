...
//Ausprobieren was passiert, wenn ich beim wechseln nicht docker-compose build benutze

IMPORTANT: Falls beim Update das Wiederaufspielen der vorherigen Version geplant ist, dann muss hierfür eine zweite Instanz erstellt werden. Achte darauf, das Image beim wechseln der Instanzen mit `docker-compose build` neu zu erstellen, da


//IMPORTANT: Falls das Update lediglich für Testzwecke gedacht ist, verwenden Sie nicht den Original branch (klaros oder master), da sonst die Konfigurationen der vorherigen Version verloren geht (Hierfür kann das Verzeichnis kopiert werden). Achten Sie ebenfalls darauf, ein anderes Volume zu verwenden und das alte Image nach dem Testen mit "docker-compose build" neu zu erstellen, damit die Originalversion nicht versehentlich aktualisiert wird. 
